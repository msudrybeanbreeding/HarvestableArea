/*******************************************************************************
* Program: Multitemporal Vegetative Index Correlation Analysis
...
%MACRO corr_analysis(input_data=, das_value=, trial_value=, output_suffix=);
    ...
    DATA temp_data;
        ...
        /* Keep only relevant variables for correlation */
        KEEP Trial DAS 
             ExG_Mean ExG_Var 
             ExR_Mean ExR_Var 
             NDVI_Mean NDVI_Var 
...
PROC EXPORT DATA=all_correlations
    ...
    DBMS=CSV
    REPLACE;/* Prepare data for heatmap visualization */
PROC SORT DATA=all_wholistic_corr;
...
* 
* 1. Update file paths:
*    - Line 20: Input CSV file path
*    - Lines 316-326: Output CSV file paths
*
...
*    - Exported CSV files
*
* 4. To run specific sections, submit individual PROC steps as needed
*
* 5. Adjust ODS output destinations if you want different output formats
*    (PDF, RTF, HTML, etc.)
*******************************************************************************/
