/*******************************************************************************
* Program: Multitemporal Vegetative Index Correlation Analysis (CORRECTED v2)
...
* 
* CORRECTIONS: 
* - Fixed variable renaming issue with HA_Mean and PLT_COUNT
* - Fixed PROC MEANS to use correct variable names from correlation output
*******************************************************************************/
ods graphics on;
 dm 'log; clear;';
...
* CORRECTED: Changed VAR statement to use correct correlation column name
*******************************************************************************/

/* Calculate mean correlations by trial across all DAS */
PROC MEANS DATA=all_trial_corr N MEAN STD MIN MAX NOPRINT;
    TITLE2 "Summary Statistics of Correlations by Trial";
    CLASS Trial_ID Variable;
    VAR Yield;  /* Yield column contains the correlation values */
    OUTPUT OUT=trial_summary MEAN=Mean_Corr STD=Std_Corr N=N_Obs;
RUN;

/* Display the summary with better formatting */
PROC PRINT DATA=trial_summary NOOBS;
    TITLE2 "Mean Correlation with Yield by Trial and Variable";
    WHERE _TYPE_ = 3;  /* Both Trial_ID and Variable specified */
    VAR Trial_ID Variable Mean_Corr Std_Corr N_Obs;
    FORMAT Mean_Corr Std_Corr 8.4;
RUN;

/* Alternative: Create a simpler summary without CLASS statement if above fails */
PROC SQL;
    CREATE TABLE trial_summary_simple AS
    SELECT 
        Trial_ID,
        Variable,
        MEAN(Yield) AS Mean_Corr,
        STD(Yield) AS Std_Corr,
        COUNT(*) AS N_Obs,
        MIN(Yield) AS Min_Corr,
        MAX(Yield) AS Max_Corr
    FROM all_trial_corr
    WHERE Trial_ID IS NOT MISSING
    GROUP BY Trial_ID, Variable
    ORDER BY Trial_ID, Variable;
QUIT;

PROC PRINT DATA=trial_summary_simple NOOBS;
    TITLE2 "Mean Correlation with Yield by Trial and Variable (Alternative View)";
    VAR Trial_ID Variable Mean_Corr Std_Corr N_Obs Min_Corr Max_Corr;
    FORMAT Mean_Corr Std_Corr Min_Corr Max_Corr 8.4;
RUN;

/*******************************************************************************
* STEP 13: Additional Summary - Average Correlation by Variable Across All Trials
*******************************************************************************/

PROC SQL;
    CREATE TABLE variable_summary AS
    SELECT 
        Variable,
        MEAN(Yield) AS Avg_Corr_AllTrials,
        STD(Yield) AS Std_Corr_AllTrials,
        COUNT(*) AS N_Observations
    FROM all_trial_corr
    GROUP BY Variable
    ORDER BY Avg_Corr_AllTrials DESC;
QUIT;

PROC PRINT DATA=variable_summary NOOBS;
    TITLE2 "Average Correlation by Variable Across All Trials and DAS";
    VAR Variable Avg_Corr_AllTrials Std_Corr_AllTrials N_Observations;
    FORMAT Avg_Corr_AllTrials Std_Corr_AllTrials 8.4;
RUN;

/*******************************************************************************
* STEP 14: Clean up temporary datasets
*******************************************************************************/
PROC DATASETS LIBRARY=WORK NOLIST;
...
* - Fixed PROC MEANS to correctly reference Yield column (which contains correlations)
* - Added alternative PROC SQL summary in case PROC MEANS has issues
* 
...
* - Variable: Name of the VI variable being correlated
* - Yield: The correlation coefficient value
* - PYield: P-value for the correlation
* - nYield: Number of observations used in correlation
*
* OUTPUTS GENERATED:
...
* - Summary statistics by trial and variable
